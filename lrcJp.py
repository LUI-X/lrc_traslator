...
class LrcFile:
    ...
    def orderLines(self):
        for key, list in self.keyAndList.items():
            ...
            tokenizedLyrics = self.jpTokenizer(lyrics)
            lyricsConverted = self.jpConvertor(lyrics)
            traduccion = translator.translate(lyrics, dest='es')

            self.newLyrics.extend(('[' + minutes + ':' + seconds + '.' + milliseconds + ']' + tokenizedLyrics))
            nextTime = self.getNextTime(key)
            timeTra = self.setTime(nextTime[0], nextTime[1], nextTime[2])
            timeHi = self.setTime(timeTra[0], timeTra[1], timeTra[2])
            timeKana = self.setTime(timeHi[0], timeHi[1], timeHi[2])

            self.newLyrics.extend(('[' + timeKana[0] + ':' + timeKana[1] + '.' + timeKana[2] + ']' + "(ka:" + lyricsConverted[0].rstrip("\n") + ")"))
            self.newLyrics.extend(('\n'))
            self.newLyrics.extend(('[' + timeHi[0] + ':' + timeHi[1] + '.' + timeHi[2] + ']' + "(hi:" + lyricsConverted[1].rstrip("\n") + ")"))
            self.newLyrics.extend(('\n'))
...
titleLrc = "files/Gundam Wing Opening 1 JUST COMMUNICATION + LYRICS.lrc"
